...
class OrganizationController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        $model = $this->findModel($id);
		$events = Events::find()->all();
		$cheked = EventsOrganization::find()->select('events_id')->where(['organization_id' => $model->id])->indexBy('events_id')->column();
		...
		*/
        ...
        if ($this->request->isPost) {
			...
			foreach($data['events'] as $item) {
				EventsOrganization::deleteAll(['organization_id' => $model->id]);
				$link = new EventsOrganization();
				$link->organization_id = $model->id;
				$link->events_id = $item;
				$link->save();
